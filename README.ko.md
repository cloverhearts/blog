...
페이지에서 두 쿠키 저장 목적을 모두 거부한 상태로 쿠키 없는 제한적 분석을
자동 시작합니다. 텍스트와 입력값은 가리고 검색·미리보기·관리 페이지는 제외합니다.
접힌 분석 안내에서 수집을 중지할 수 있으며 기존 거부 선택은 유지합니다.
재방문자 식별과 페이지 간 이동 분석은 제한됩니다.
