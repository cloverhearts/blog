...
通常のブログには、読者の同意を条件とする GA4 アダプターがあります。有効にするには、
次の公開ビルド値を追加します。
...
値を空にすると分析機能は完全に無効になります。設定値はビルド時に検証され、読者が同意する
前に Google への接続は行いません。URL のクエリや生の検索語は収集せず、管理ページは
既定で追跡しません。GitHub Pages では、`GA4_MEASUREMENT_ID` というリポジトリ変数、
または `github-pages` 環境の変数として保存してください。
