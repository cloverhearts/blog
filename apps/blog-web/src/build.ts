...
function renderShell(
  ...
  const consent = analyticsEnabled
      ? `<details class="analytics-consent" data-analytics-consent data-pagefind-ignore data-clarity-project="${escape(config.resolved.clarity.projectId!)}" data-consent-key="${escape(config.analytics.consent.storageKey)}"><summary class="site-control">${escape(messages.analyticsNotice)}</summary><p>${escape(messages.analyticsDisclosure)} <a href="https://privacy.microsoft.com/privacystatement" rel="noreferrer">${escape(messages.analyticsPrivacy)}</a></p><button class="site-control site-control--button analytics-consent__deny" type="button" data-analytics-deny aria-pressed="false">${escape(messages.denyAnalytics)}</button></details>`
      : "";
